...
│   ├── app/
│   │   ├── (public)/page.tsx ✅ (harta statike), (public)/zones/[id]/ ✅ (detaje+listë spotesh+rezervim), (public)/reservations/ ✅ (rezervimet e mia)
...
│   ├── hooks/{useZones.ts ✅ (+mutime admin),useSpots.ts ✅ (+mutime admin),useParkingSocket.ts ✅ (patch cache live),useSessions.ts ✅,useReservations.ts ✅,useAdminStats.ts ✅}
│   ├── lib/{api.ts ✅ (auth header+refresh-retry),auth.ts ✅,sessions.ts ✅,reservations.ts ✅,admin.ts ✅,zones.ts ✅ (+CRUD shkrimi),spots.ts ✅ (+CRUD shkrimi),types.ts,socket.ts,utils.ts}
...
│   │   │   └── {notifications,audit-log}/ (bosh, fazë e mëvonshme)
