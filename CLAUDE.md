...
│   │   ├── (dashboard)/admin/ ✅ (stats+zones-panel+spots-panel, rol-gated), (dashboard)/attendant/
│   │   └── layout.tsx
│   ├── components/{map/ParkingMap.tsx ✅ (me flash live),nav/Header.tsx ✅ (zile njoftimesh live),ui,providers}/
│   ├── hooks/{useZones.ts ✅ (+mutime admin),useSpots.ts ✅ (+mutime admin),useParkingSocket.ts ✅ (patch cache live),useSessions.ts ✅,useReservations.ts ✅,useAdminStats.ts ✅,useNotifications.ts ✅ (+socket privat live)}
│   ├── lib/{api.ts ✅ (auth header+refresh-retry),auth.ts ✅,sessions.ts ✅,reservations.ts ✅,admin.ts ✅,notifications.ts ✅,zones.ts ✅ (+CRUD shkrimi),spots.ts ✅ (+CRUD shkrimi),types.ts,socket.ts ✅ (auth JWT në handshake)}
...
│   │   │   └── audit-log/ (bosh, fazë e mëvonshme)
