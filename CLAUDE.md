...
│   │   ├── (dashboard)/admin/ ✅ (stats+zones-panel+spots-panel+audit-log-panel, rol-gated), (dashboard)/attendant/
│   │   └── layout.tsx
│   ├── components/{map/ParkingMap.tsx ✅ (me flash live),nav/Header.tsx ✅ (zile njoftimesh live),ui,providers}/
│   ├── hooks/{useZones.ts ✅ (+mutime admin, invalidon auditLogs),useSpots.ts ✅ (+mutime admin, invalidon auditLogs),useParkingSocket.ts ✅ (patch cache live),useSessions.ts ✅,useReservations.ts ✅,useAdminStats.ts ✅,useNotifications.ts ✅ (+socket privat live),useAuditLogs.ts ✅}
│   ├── lib/{api.ts ✅ (auth header+refresh-retry),auth.ts ✅,sessions.ts ✅,reservations.ts ✅,admin.ts ✅,notifications.ts ✅,zones.ts ✅ (+CRUD shkrimi),spots.ts ✅ (+CRUD shkrimi),audit-log.ts ✅,types.ts,socket.ts ✅ (auth JWT në handshake)}
...
│   │   │   ├── admin/ ✅ (GET /admin/stats — admin-only, groupBy Prisma pa SQL raw)
